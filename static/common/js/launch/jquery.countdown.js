...
var createDigits = function(where, options) {
    ...
    // if imgs_separators is an array :
    if (Object.prototype.toString.call(
            options.imgs_separators) === '[object Array]'
    ) {
        // ... and it's not empty:
        if (options.imgs_separators.length>0) {
            var currImgsSeparator=0;
        }
    }
    for (var i = 0; i < options.startTime.length; i++) {
        var curr=options.startTime[i];
        if (parseInt(curr) >= 0) {
            var elem = $('<div id="cnt_' + counter + '" class="cntDigit" />')
                .css({
            ...
            if (options.continuous === true) {
                ...
                // reach the max. For example second "hours" digit has a
                // conditional max of 4
            ...
            counter += 1;
        } else if (typeof(currImgsSeparator)=='number') {
            var bgImg = options.imgs_separators[currImgsSeparator];
            if (typeof(bgImg)=='string') {
                var elem = $('<div class="cntDigit" />')
                    .attr('id', 'cntSeparator-'+currImgsSeparator)
                    .css({
                        height: options.digitHeight,
                        float: 'left',
                        background: 'url(\'' + bgImg + '\')',
                        width: (currImgsSeparator>0 ?
                            options.digitWidth :
                            parseInt(options.digitWidth))
                    });
                if (currImgsSeparator==0) {
                    if (options.forMobile) {
                        elem=$('<div style="float: left; width: 44px" />')
                            .append(elem);
                    } else {
                        elem=$('<div style="float: left; width: 100px" />')
                            .append(elem);
                    }
                }
                currImgsSeparator++;
                if (currImgsSeparator == options.imgs_separators.length) {
                    currImgsSeparator = 0;
                }
            }
        } else {
            var elem = $('<div class="cntSeparator"/>')
                .css({float: 'left'})
                .text(curr);
        }
        where.append(elem)
