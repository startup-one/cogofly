...
class MockOpener(object):
    ...
    def open(self, request, data=None, timeout=None):
        if self.response.code != 200:
            raise six.moves.urllib.error.URLError(self.response.code)
...
class RemotingServiceTestCase(BaseServiceTestCase):
    ...
    def test_get_amf_request(self):
        ...
        self.assertEqual(list(envelope.keys()), ['/1'])
        ...
        self.assertEqual(list(envelope2.keys()), ['/1'])
