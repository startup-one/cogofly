...
class EncoderTestCase(ClassCacheClearingTestCase, EncoderMixIn):
    ...
    def test_dict(self):
        ...
        self.assertEncoded({'a': 'e', 'b': 'f', 'c': 'g', 'd': 'h'},
...
class DecoderTestCase(ClassCacheClearingTestCase, DecoderMixIn):
    ...
    def test_mixed_array(self):
        ...
        self.assertEqual(y,
            {'a': 'a', 'b': 'b', 'c': 'c', 'd': 'd', 0: 0, 1: 1, 2: 2, 3: 3})
    ...
    def test_dict(self):
        self.assertDecoded({'a': 'a', 'b': 'b', 'c': 'c', 'd': 'd'},
            '\n\x0b\x01\x03a\x06\x00\x03c\x06\x02\x03b\x06\x04\x03d\x06\x06\x01')

        self.assertDecoded({0: 'hello', 'foo': 'bar'}, '\x09\x03\x07\x66\x6f'
        ...
        self.assertDecoded({'a': 1, 'b': 2}, '\x0a\x0b\x01\x03\x62\x04\x02\x03'
            '\x61\x04\x01\x01')
        self.assertDecoded({'baz': 'hello'}, '\x0a\x0b\x01\x07\x62\x61\x7a'
            '\x06\x0b\x68\x65\x6c\x6c\x6f\x01')
        self.assertDecoded({'baz': 'hello'}, '\x0a\x13\x01\x07\x62\x61\x7a'
    ...
    def test_ioerror_buffer_position(self):
        ...
        """
        bytes = pyamf.encode('foo', [1, 2, 3], encoding=pyamf.AMF3).getvalue()
...
class DataOutputTestCase(unittest.TestCase, EncoderMixIn):
    ...
    def test_multi_byte(self):
        ...
        self.assertEqual(self.buf.getvalue(), 'this is a test')
        self.buf.truncate()

        self.x.writeMultiByte('ἔδωσαν', 'utf-8')
...
class DataInputTestCase(unittest.TestCase):
    ...
    def test_multi_byte(self):
        ...
        self._test('\xe1\xbc\x94\xce\xb4\xcf\x89\xcf\x83\xce\xb1\xce\xbd',
            'ἔδωσαν', x.readMultiByte, 13, 'utf-8')
    ...
    def test_utf(self):
        ...
        self._test('\x00\x0bhello world', 'hello world', x.readUTF)
