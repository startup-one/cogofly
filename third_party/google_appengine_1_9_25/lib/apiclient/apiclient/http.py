...
class HttpRequest(object):
  ...
  def execute(self, http=None, num_retries=0):
    ...
    if len(self.uri) > MAX_URI_LENGTH and self.method == 'GET':
      ...
      parsed = six.moves.urllib.parse.urllib.parse(self.uri)
      self.uri = six.moves.urllib.parse.urlunparse(
...
class BatchHttpRequest(object):
  ...
  def _id_to_header(self, id_):
    ...
    return '<%s+%s>' % (self._base_id, six.moves.urllib.parse.quote(id_))
  ...
  def _header_to_id(self, header):
    ...
    return six.moves.urllib.parse.unquote(id_)
  ...
  def _serialize_request(self, request):
    ...
    parsed = six.moves.urllib.parse.urllib.parse(request.uri)
    request_line = six.moves.urllib.parse.urlunparse(
