...
class BaseFault(object):
    ...
    def raiseException(self):
        ...
        available.
        """
        six.reraise(get_exception_from_fault(self), self.description, None)
...
def _read_body(stream, decoder, strict=False, logger=None):
    ...
    for code, s in six.iteritems(STATUS_CODES):
...
def _write_body(name, message, stream, encoder, strict=False):
    ...
    if isinstance(message, Request):
        target = six.text_type(message.target)
    else:
...
def encode(msg, strict=False, logger=None, timezone_offset=None):
    ...
    for name, header in six.iteritems(msg.headers):
        _write_header(
            name,
            header,
    ...
    for name, message in six.iteritems(msg):
