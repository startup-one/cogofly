...
class BaseTestCase(unittest.TestCase):
    ...
    def create_movie_data(self):
        ...
        alien = Movie(
            ...
            year=1979,
            director=rscott,
            genres=[scifi, Genre(name="Horror")]
        )
        brunner = Movie(
            title="Blade Runner", year=1982, director=rscott, genres=[scifi]
        )
        swars = Movie(
            title="Star Wars", year=1977, director=glucas, genres=[scifi]
        )
...
class ClassAliasTestCase(BaseTestCase):
    ...
    def test_get_attrs(self):
        m = Movie.query.filter_by(title="Blade Runner").one()
        ...
        self.assertEqual(attrs, {
            ...
            'director': d,
            'year': 1982,
            'sa_key': ['Blade Runner', 1982],
            'sa_lazy': []
        })

    def test_inheritance(self):
        d = Director.query.filter_by(name="Ridley Scott").one()
