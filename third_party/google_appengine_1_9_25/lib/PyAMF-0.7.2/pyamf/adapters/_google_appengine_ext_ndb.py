...
class NDBStubCollection(gae_base.StubCollection):
    def fetchEntities(self):
        return dict(list(zip(self.to_fetch, ndb.get_multi(self.to_fetch))))
...
class NDBClassAlias(gae_base.BaseDatastoreClassAlias):
    ...
    def getCustomProperties(self):
        ...
        for name, prop in six.iteritems(self.klass._properties):
            props[name] = prop
        ...
        for name, value in six.iteritems(self.klass.__dict__):
            if isinstance(value, ndb.ComputedProperty):
                read_only_props.append(name)

        self.encodable_properties.update(list(props.keys()))
        self.decodable_properties.update(list(props.keys()))
        self.readonly_attrs.update(read_only_props)

        if computed_props:
            self.decodable_properties.difference_update(list(computed_props.keys()))
    ...
    def getDecodableAttributes(self, obj, attrs, codec=None):
        ...
        if self.repeated_properties:
            for name, prop in six.iteritems(self.repeated_properties):
                try:
                    value = attrs[name]
                except KeyError:
                    continue
    ...
    def getEncodableAttributes(self, obj, codec=None):
        ...
        if isinstance(obj, ndb.Expando):
            for name, prop in six.iteritems(obj._properties):
