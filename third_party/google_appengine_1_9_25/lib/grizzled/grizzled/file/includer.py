...
class Includer(object):
    ...
    def __open(self, name_to_open, enclosing_file, enclosing_file_is_url):
        ...
        parsed_url = six.moves.urllib.parse.urllib.parse(name_to_open)

        # Account for Windows drive letters.

        if (parsed_url.scheme != '') and (len(parsed_url.scheme) > 1):
            openFunc = six.moves.urllib.request.urlopen
            is_url = True
            ...
            if enclosing_file_is_url:
                ...
                name_to_open = six.moves.urllib.parse.urljoin(enclosing_file, name_to_open)
                open_func = six.moves.urllib.request.urlopen
                is_url = True
