...
def download(url, directory=None, bufsize=8192):
    ...
    """
    pieces = six.moves.urllib.parse.urllib.parse(url)
    path = pieces.path
    if not directory:
    ...
    if pieces.scheme.startswith('http') and pieces.username:
        ...
        url = six.moves.urllib.parse.urlunparse(newPieces)
        log.debug('Installing authorization handler for URL %s' % url)
        passwordMgr = six.moves.urllib.request.HTTPPasswordMgrWithDefaultRealm()
        passwordMgr.add_password(realm=None,
                                 uri=url,
                                 user=user,
                                 passwd=password)
        authHandler = six.moves.urllib.request.HTTPBasicAuthHandler(passwordMgr)
    ...
    shutil.copyfileobj(six.moves.urllib.request.urlopen(url), open(outputPath, 'wb'), bufsize)
