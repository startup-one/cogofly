...
class TestFTPListParse(object):
    ...
    def parse_one_line(self, parser, test_data, identifier):
        ...
        self.assertEqual(result.raw_line, line, prefix)
        self.assertEqual(result.size, test_data['size'], prefix)
        self.assertEqual(result.name, name, prefix)
        self.assertEqual(result.try_cwd, test_data['try_cwd'], prefix)
        expected_time = time.mktime(test_data['time'])
        self.assertEqual(time.localtime(result.mtime),
                          time.localtime(expected_time),
                          prefix)
        self.assertEqual(result.mtime, expected_time, prefix)
