...
import cgi, copy, sys, warnings, six.moves.urllib.request, six.moves.urllib.parse, six.moves.urllib.error
...
class MultiDict(DictMixin):
    ...
    def __init__(self, *args, **kw):
        ...
        if args:
            if hasattr(args[0], 'iteritems'):
                items = list(six.iteritems(args[0]))
    ...
    def __repr__(self):
        items = list(map('(%r, %r)'.__mod__, _hide_passwd(six.iteritems(self))))
...
class UnicodeMultiDict(DictMixin):
    ...
    def getall(self, key):
        ...
        Return a list of all values matching the key (may be an empty list)
        """
        return list(map(self._decode_value, self.multi.getall(self._encode_key(key))))
    ...
    def keys(self):
        return [self._decode_key(k) for k in six.iterkeys(self.multi)]

    def iterkeys(self):
        for k in six.iterkeys(self.multi):
    ...
    def values(self):
        return [self._decode_value(v) for v in six.itervalues(self.multi)]

    def itervalues(self):
        for v in six.itervalues(self.multi):
