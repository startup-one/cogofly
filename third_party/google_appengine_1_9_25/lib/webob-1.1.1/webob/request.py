...
import six.moves.urllib.request, six.moves.urllib.parse, six.moves.urllib.error, six.moves.urllib.parse, cgi
import six
...
class BaseRequest(object):
    ...
    def application_url(self):
        ...
        """
        return self.host_url + six.moves.urllib.parse.quote(
    ...
    def path_url(self):
        ...
        """
        return self.application_url + six.moves.urllib.parse.quote(
    ...
    def path(self):
        ...
        """
        return (six.moves.urllib.parse.quote(self.script_name, PATH_SAFE) +
                six.moves.urllib.parse.quote(self.path_info, PATH_SAFE))
    ...
    def _update_get(self, vars, key=None, value=None):
        env = self.environ
        qs = six.moves.urllib.parse.urlencode(list(vars.items()))
        env['QUERY_STRING'] = qs
    ...
    def _copy_body_tempfile(self):
        ...
        assert isinstance(todo, six.integer_types), repr(todo)
    ...
    def as_string(self, skip_body=False):
        ...
        parts += list(map('%s: %s'.__mod__, sorted(self.headers.items())))
        if body:
            parts.extend( ['',body] )
    ...
    def blank(cls, path, environ=None, base_url=None,
        ...
        if base_url:
            scheme, netloc, path, query, fragment = six.moves.urllib.parse.urlsplit(base_url)
            ...
            if path:
                env['SCRIPT_NAME'] = six.moves.urllib.parse.unquote(path)
        if environ:
            env.update(environ)
...
def environ_from_url(path):
    if SCHEME_RE.search(path):
        scheme, netloc, path, qs, fragment = six.moves.urllib.parse.urlsplit(path)
        if fragment:
    ...
    if path and '?' in path:
        ...
        path_info = six.moves.urllib.parse.unquote(path_info)
    else:
        path_info = six.moves.urllib.parse.unquote(path)
        query_string = ''
    env = {
        'REQUEST_METHOD': 'GET',
        'SCRIPT_NAME': '',
...
def environ_add_POST(env, data, content_type=None):
    ...
    if hasattr(data, 'items'):
        ...
        data.sort()
        has_files = [_ for _ in data if isinstance(_[1], (tuple, list))]
    ...
    def _get_body(self):
        if self._body is None:
            if self.content_type.startswith('application/x-www-form-urlencoded'):
                self._body = six.moves.urllib.parse.urlencode(list(self.vars.items()))
            elif self.content_type.startswith('multipart/form-data'):
                self._body = _encode_multipart(six.iteritems(self.vars), self.content_type)[1]
            else:
