...
def _add_query_parameter(url, name, value):
  ...
  if value is None:
    ...
    parsed = list(six.moves.urllib.parse.urllib.parse(url))
    q = parse_qsl(parsed[4])
    q.append((name, value))
    parsed[4] = six.moves.urllib.parse.urlencode(q)
    return six.moves.urllib.parse.urlunparse(parsed)
...
def createResource(http, baseUrl, model, requestBuilder,
  ...
  def createMethod(theclass, methodName, methodDesc, futureDesc):
    ...
    if 'parameters' in methodDesc:
      for arg, desc in six.iteritems(methodDesc['parameters']):
        param = key2param(arg)
        argmap[param] = arg
    ...
    if len(argmap) > 0:
      docs.append('Args:\n')
    for arg in six.iterkeys(argmap):
  ...
  def createNextMethod(theclass, methodName, methodDesc, futureDesc):
    ...
    def methodNext(self, previous_request, previous_response):
      ...
      parsed = list(six.moves.urllib.parse.urllib.parse(request.uri))
      q = parse_qsl(parsed[4])

      # Find and remove old 'pageToken' value from URI
      newq = [(key, value) for (key, value) in q if key != 'pageToken']
      newq.append(('pageToken', pageToken))
      parsed[4] = six.moves.urllib.parse.urlencode(newq)
  ...
  if futureDesc and 'methods' in futureDesc:
    for methodName, methodDesc in six.iteritems(futureDesc['methods']):
