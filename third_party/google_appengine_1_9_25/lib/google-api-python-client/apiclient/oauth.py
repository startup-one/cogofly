...
def _oauth_uri(name, discovery, params):
  ...
  return discovery[name]['url'] + '?' + six.moves.urllib.parse.urlencode(query)
  ...
  def authorize(self, http):
    ...
    def new_request(uri, method='GET', body=None, headers=None,
      ...
      while response_code in [301, 302]:
        ...
        parsed = list(six.moves.urllib.parse.urllib.parse(uri))
        q = parse_qsl(parsed[4])
        q.append(('xoauth_requestor_id', self._requestor))
        parsed[4] = six.moves.urllib.parse.urlencode(q)
        uri = six.moves.urllib.parse.urlunparse(parsed)
...
class FlowThreeLegged(Flow):
  ...
  def __init__(self, discovery, consumer_key, consumer_secret, user_agent,
    ...
    **kwargs        - The keyword arguments are all optional and required
    ...
    self.request_token = {}
    required = {}
    for uriinfo in six.itervalues(discovery):
      for name, value in six.iteritems(uriinfo['parameters']):
        if value['required'] and not name.startswith('oauth_'):
          required[name] = 1
    for key in six.iterkeys(required):
  ...
  def step1_get_authorize_url(self, oauth_callback='oob'):
    ...
    body = six.moves.urllib.parse.urlencode({'oauth_callback': oauth_callback})
  ...
  def step2_exchange(self, verifier):
    ...
    if not (isinstance(verifier, str) or isinstance(verifier, six.text_type)):
