...
def _serialize_date(dt):
    if dt is None:
        return None
    if isinstance(dt, six.text_type):
        dt = dt.encode('ascii')
...
class Request(object):
    ...
    def application_url(self):
        ...
        """
        return self.host_url + six.moves.urllib.parse.quote(self.environ.get('SCRIPT_NAME', ''))
    ...
    def path_url(self):
        ...
        """
        return self.application_url + six.moves.urllib.parse.quote(self.environ.get('PATH_INFO', ''))
    ...
    def path(self):
        ...
        """
        return six.moves.urllib.parse.quote(self.script_name) + six.moves.urllib.parse.quote(self.path_info)
...
class Response(object):
    ...
    def _location__get(self):
        ...
        if self.request is not None:
            ...
            location = six.moves.urllib.parse.urljoin(base_uri, location)
        return location
    ...
    def _location__set(self, value):
        if not _SCHEME_RE.search(value):
            ...
            if self.request is not None:
                value = six.moves.urllib.parse.urljoin(self.request.url, value)
