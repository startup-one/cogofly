...
class _Remapped405Exception(api_exceptions.ServiceException):
  ...
  http_status = six.moves.http_client.METHOD_NOT_ALLOWED
...
class _Remapped408Exception(api_exceptions.ServiceException):
  ...
  http_status = six.moves.http_client.REQUEST_TIMEOUT


_ERROR_NAME_MAP = dict((six.moves.http_client.responses[c.http_status], c) for c in [
...
class _ApiServer(object):
  ...
  def __register_services(api_name_version_map, api_config_registry):
    ...
    protorpc_services = []
    for service_factories in six.itervalues(api_name_version_map):
  ...
  def __write_error(self, status_code, error_message=None):
    ...
    if error_message is None:
      error_message = six.moves.http_client.responses[status_code]
    status = '%d %s' % (status_code, six.moves.http_client.responses[status_code])
  ...
  def __call__(self, environ, start_response):
    ...
    if self.__is_request_restricted(environ):
      status, body = self.__write_error(six.moves.http_client.NOT_FOUND)
