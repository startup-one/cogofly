...
import logging
import socket
from six.moves.urllib.parse import splitpasswd
...
def create_fancy_connection(tunnel_host=None, key_file=None,
  ...
  class PresetProxyHTTPSConnection(six.moves.http_client.HTTPSConnection):
    ...
    def connect(self):
      ...
      if can_validate_certs():
        ...
        ssl_socket = socket.ssl(self.sock,
                                keyfile=self.key_file,
                                certfile=self.cert_file)
        self.sock = six.moves.http_client.FakeSocket(self.sock, ssl_socket)
...
class FancyRequest(six.moves.urllib.request.Request):
  ...
  def set_proxy(self, host, type):
    ...
    six.moves.urllib.request.Request.set_proxy(self, host, type)
...
class FancyProxyHandler(six.moves.urllib.request.ProxyHandler):
  ...
  def proxy_open(self, req, proxy, type):
    ...
    if user and password:
      user_pass = "%s:%s" % (six.moves.urllib.parse.unquote(user), six.moves.urllib.parse.unquote(password))
      creds = base64.b64encode(user_pass).strip()
      # Later calls overwrite earlier calls for the same header
      req.add_header("Proxy-authorization", "Basic " + creds)
    hostport = six.moves.urllib.parse.unquote(hostport)
    req.set_proxy(hostport, proxy_type)
    # This condition is the change
    if orig_type == "https":
      return None

    return six.moves.urllib.request.ProxyHandler.proxy_open(self, req, proxy, type)


class FancyHTTPSHandler(six.moves.urllib.request.HTTPSHandler):
  ...
  def do_open(self, http_class, req, *args, **kwargs):
    ...
    try:
      return six.moves.urllib.request.HTTPSHandler.do_open(
          self,
...
# through redirects.
class FancyRedirectHandler(six.moves.urllib.request.HTTPRedirectHandler):
  """A redirect handler that persists CONNECT-enabled proxy information."""

  def redirect_request(self, req, *args, **kwargs):
    new_req = six.moves.urllib.request.HTTPRedirectHandler.redirect_request(
    ...
    if hasattr(req, "_tunnel_host") and isinstance(new_req, six.moves.urllib.request.Request):
      if new_req.get_type() == "https":
        ...
        new_req.type = "https"
    if hasattr(req, "_key_file") and isinstance(new_req, six.moves.urllib.request.Request):
