...
class Cookie(dict):
    ...
    def serialize(self, full=True):
        return '; '.join(m.serialize(full) for m in list(self.values()))
    ...
    def __repr__(self):
        return '<%s: [%s]>' % (self.__class__.__name__,
                               ', '.join(map(repr, list(self.values()))))
...
_escape_map.update(list(zip(_escape_noop_chars, _escape_noop_chars)))
_escape_map['"'] = r'\"'
_escape_map['\\'] = r'\\'
if PY3: # pragma: no cover
    # convert to {int -> bytes}
    _escape_map = dict((ord(k), bytes_(v, 'ascii')) for k, v in list(_escape_map.items()))
