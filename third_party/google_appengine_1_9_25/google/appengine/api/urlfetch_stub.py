  ...
  def _RetrieveURL(url, payload, method, headers, request, response,
    ...
    for redirect_number in range(MAX_REDIRECTS + 1):
      ...
      try:
        ...
        if protocol == 'http':
          ...
          if os.environ.get('HTTP_PROXY') and not _IsLocalhost(host):
            _, proxy_host, _, _, _ = (
                six.moves.urllib.parse.urlsplit(os.environ.get('HTTP_PROXY')))
        elif protocol == 'https':
          ...
          if (_CONNECTION_SUPPORTS_SSL_TUNNEL and
            ...
            _, proxy_host, _, _, _ = (
                six.moves.urllib.parse.urlsplit(os.environ.get('HTTPS_PROXY')))
        else:
        ...
        if proxy_host:
          ...
          connection = connection_class(
              ...
              **connection_kwargs)
          full_path = six.moves.urllib.parse.urlunsplit((protocol, host, path, query, ''))

          if protocol == 'https':
            connection.set_tunnel(host)
        else:
          connection = connection_class(host, **connection_kwargs)
          full_path = six.moves.urllib.parse.urlunsplit(('', '', path, query, ''))
      ...
      if http_response.status in REDIRECT_STATUSES and follow_redirects:
        ...
        if (http_response.status != six.moves.http_client.TEMPORARY_REDIRECT and
