...
class UserServiceStub(apiproxy_stub.APIProxyStub):
  ...
  def _AddHostToContinueURL(self, continue_url, request_id):
    ...
    Returns:
      string
    """
    (protocol, host, path, parameters, query, fragment) = six.moves.urllib.parse.urllib.parse(continue_url)

    if host and protocol:
      return continue_url

    try:
      protocol, host, _, _, _, _ = six.moves.urllib.parse.urllib.parse(
